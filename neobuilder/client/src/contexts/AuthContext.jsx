...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const requireManualLogin = localStorage.getItem('requireLoginAfterReactivation') === '1';
  ...
  const login = async (payload) => {
    ...
    localStorage.removeItem('requireLoginAfterReactivation');
  ...
  return (
    <AuthContext.Provider value={{ user, setUser, loading, manualLoginRequired, login, signup, logout, refreshUser }}>
      {children}
