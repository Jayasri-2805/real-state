...
import { Plus, Pencil, Trash2, X, Lock, Search } from 'lucide-react';
  ...
  return (
    <div className="page">
      <div style={{ fontSize: '14px', color: 'var(--text-muted)', marginBottom: '24px' }}>
        <span>Home</span> / <span>Administration</span> / <span style={{ color: 'var(--text)' }}>Roles</span>
      </div>
      ...
      {portalTarget && createPortal(
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'nowrap' }}>
          ...
          <button className="btn btn-primary header-add-btn" onClick={() => { setEditingRole(null); setModalOpen(true); }}>
            <Plus size={15} /> <span className="btn-text">Add Role</span>
          </button>
        </div>,
        portalTarget
      )}
