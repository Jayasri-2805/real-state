...
export default function PurchaseIndentPage() {
  ...
  useEffect(() => {
    ...
    if (params.get('add') === '1' || (location.state && location.state.openAdd)) {
      setInitialData(location.state?.initialData || null);
      setShowAddModal(true);
    }
  }, [location.search, location.state]);

  if (showAddModal && location.search.includes('add=1')) {
    return (
      <div className="page">
        <AddIndentModal
  ...
  return (
    <div className="page">
      ...
      {portalTarget && createPortal(
        <button className="header-add-rejected" onClick={() => setShowAddModal(true)}>
