...
export async function updateStatus(req, res, next) {
  try {
    ...
    if (!user) throw new ApiError(404, 'User not found');
    if (user.isOwner) {
      throw new ApiError(403, 'The company owner status cannot be changed');
    }
    ...
    if (previousStatus !== 'Inactive' && status === 'Inactive') {
    ...
    return success(res, { message: 'Status updated successfully', data: user });
  } catch (err) {
    next(err);
  }
}
